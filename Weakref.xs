...
static void clone_array (SV * ref, SV *target) {
    I32 arrlen = 0;
    AV *original = (AV *)ref;
    AV *clone = (AV *)target;
    SV **svp;
...
static SV * clone_value (SV *original, UV visible) {
    ...
    /* 
       ...
       though its refcount is fixed at 1 (< 5.8.0) or 2. In this case 'visible'
...
=cut
    ...
    if (SvRMAGICAL(original) || SvGMAGICAL(original)) {
		...
		*/
		...
		if (clone_handler && (weakref = mg_find(original, WEAKREF_IDENTIFIER))) {
...
MODULE = Clone::Weakref		PACKAGE = Clone::Weakref		
